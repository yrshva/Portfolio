import "../styles/resume.css";

const Resume = () => {
  return (
    <div className="cv">
      <div className="c21">
        <p className="c4 title">
          <span>Anastasiia Yershova</span>
        </p>
        <p className="c2">
          <span className="c12">Manchester, UK</span>
        </p>
        <p className="c2">
          <span className="c15 c22">Email</span>
          <span className="c15">: </span>
          <span className="c5">
            <a className="c10" href="mailto:yershova.business@gmail.com">
              yershova.business@gmail.com
            </a>
          </span>
          <span className="c14">&nbsp; &nbsp;</span>
        </p>
        <p className="c2">
          <span className="c15 c22">Linkedin</span>
          <span className="c15">: </span>
          <span className="c5">
            <a
              className="c10"
              href="https://www.linkedin.com/in/anastasiia-yershova/"
            >
              https://www.linkedin.com/in/anastasiia-yershova/
            </a>
          </span>
        </p>
        <p className="c2">
          <span className="c15 c22">GitHub: </span>
          <span className="c5">
            <a className="c10" href="https://github.com/yrshva">
              https://github.com/yrshva
            </a>
          </span>
        </p>
        <p className="c2 c24">
          <span className="c14"></span>
        </p>
        <p className="c2">
          <span>
            Front-end Developer with 1 year commercial experience in web
            development, a background in UI/UX Design, and &nbsp;a 3 years
            university background of PHP, JS, HTML, CSS. Excellent in dealing
            with payment gateway integrations and other billing matters since
            I`ve got 3 years experience of billing management.
          </span>
        </p>
        <h1 className="c19 c18">
          <span className="c25 c22">Skills</span>
        </h1>
        <ul className="c11 lst-kix_cigb9kkcnmxx-0 start">
          <li className="c16 li-bullet-0">
            <span className="c7">React.js, React Native, Redux</span>
          </li>
          <li className="c16 li-bullet-0">
            <span className="c7">JavaScript, TypeScript</span>
          </li>
          <li className="c16 li-bullet-0">
            <span className="c7">REST API, Git</span>
          </li>
          <li className="c16 c18 li-bullet-0">
            <span className="c7">Bootstrap, Chakra UI, Material UI</span>
          </li>
          <li className="c16 c18 li-bullet-0">
            <span className="c7">HTML, CSS, SCSS</span>
          </li>
          <li className="c16 c18 li-bullet-0">
            <span className="c7">
              Visual design: Adobe Photoshop, Figma, Adobe XD
            </span>
          </li>
          <li className="c16 c18 li-bullet-0">
            <span>Payment gateways API research and integration</span>
          </li>
        </ul>
        <h1 className="c18 c19">
          <span className="c22 c25">Experience</span>
        </h1>
        <h2 className="c8">
          <span>October</span>
          <span className="c1">&nbsp; 2022 - Present</span>
        </h2>
        <h3 className="c0">
          <span>Interactive Investor</span>
          <span className="c23">- </span>
          <span className="c6">App Developer</span>
        </h3>
        <ul className="c11 lst-kix_iqzifbczfjv5-0 start">
          <li className="c9 li-bullet-1">
            <span>
              &nbsp;Building high-quality web, mobile-web and mobile-native UIs
              for customer and internal facing users using React and/or React
              Native, JavaScript, and TypeScript.
            </span>
          </li>
        </ul>
        <h2 className="c8">
          <span>April &nbsp;2022 - August 2022</span>
        </h2>
        <h3 className="c0">
          <span>Avanta</span>
          <span>, Kyiv</span>
          <span className="c6">
            &nbsp;- Payments Business Development Manager
          </span>
        </h3>
        <ul className="c11 lst-kix_y5lajpktzaow-0 start">
          <li className="c3">
            <span className="c7">
              Connected new payment solutions by constant research and
              negotiations
            </span>
          </li>
        </ul>
        <h2 className="c8">
          <span className="c1">November &nbsp;2021 - April 2022</span>
        </h2>
        <h3 className="c0">
          <span>Bistox Holding OU, Kyiv</span>
          <span className="c6">&nbsp;- Billing Manager</span>
        </h3>
        <ul className="c11 lst-kix_y5lajpktzaow-0">
          <li className="c3">
            <span className="c7">
              Brought sales to the company by connecting payment gateways
            </span>
          </li>
          <li className="c3">
            <span className="c7">
              Resolved payment issues with setting up routing &amp; cascading
            </span>
          </li>
        </ul>
        <h2 className="c8">
          <span>September &nbsp;2019 &nbsp;- October 2021</span>
        </h2>
        <h3 className="c0">
          <span>BI-Online Solutions</span>
          <span>, Kyiv</span>
          <span className="c6">&nbsp;- Billing Manager</span>
        </h3>
        <ul className="c11 lst-kix_y5lajpktzaow-0">
          <li className="c3">
            <span>
              Increased sales by 50% by quick research of relevant B2B or B2C
              bank account providers
            </span>
          </li>
        </ul>
        <h1 className="c19 c18">
          <span className="c25 c22">Education</span>
        </h1>
        <h2 className="c8">
          <span>September</span>
          <span className="c1">&nbsp; 2017 - February 2020</span>
        </h2>
        <h3 className="c0">
          <span>Igor Sikorsky Kyiv Polytechnic Institute</span>
          <span>, Kyiv</span>
          <span className="c17">
            &nbsp;- Bachelor&rsquo;s Degree in Software Engineering
          </span>
        </h3>
      </div>
    </div>
  );
};
export default Resume;
